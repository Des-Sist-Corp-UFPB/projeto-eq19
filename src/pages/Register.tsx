  ...
  return (
    <div className="container" style={{ maxWidth: '520px', paddingTop: '40px' }}>
      <div className="card" style={cardStyle}>
        ...
        <p style={subtitleStyle}>Cadastre-se para participar do clube e manter sua conta segura.</p>
        <form onSubmit={handleSubmit} style={formStyle}>
          ...
          <p style={tipStyle}>Use ao menos 8 caracteres, uma letra maiúscula, um número e um símbolo.</p>
          <button className="btn btn-primary" type="submit" disabled={loading}>{loading ? 'Criando conta...' : 'Criar conta'}</button>
        </form>
        <p style={linksStyle}><Link to="/login">Já tenho conta</Link></p>
      </div>
    </div>
  );
};
...
const tipStyle: React.CSSProperties = { fontSize: '0.82rem', color: 'var(--color-text-muted)' };
